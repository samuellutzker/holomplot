...
public:
    ...
    Expr() : left(nullptr), right(nullptr), value(0), op(0) {}

    Expr(Expr&&) = delete; // Disable the move constructor
    ...
    T operator()(map<string, T> vars) {
        ...
        if (!name.empty()) {
            if (funcs1.find(name) != funcs1.end()) {
                return funcs1[name]((*left)(vars));
            }
            if (funcs2.find(name) != funcs2.end()) {
                return funcs2[name]((*left)(vars), (*right)(vars));
            }
